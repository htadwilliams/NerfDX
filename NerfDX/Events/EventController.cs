...
namespace NerfDX.Events
{
    public class EventController
    {
        ...
        public override string ToString()
        {
            ...
            if (null != Joystick)
            {
                toString += " " + Joystick.Information.InstanceName;
                switch (Type)
                {
                    case EventController.EventType.Button:
                        toString += " (Button " + Button + " " + ButtonValue + ")";
                        break;

                    case EventController.EventType.POV:
                        toString += " (POV " + POVState + ")";
                        break;

                    case EventController.EventType.Axis:
                        toString += string.Format(" (Axis {0} {1:n0})", JoystickUpdate.Offset.ToString(), JoystickUpdate.Value);
                        break;
                }
            }
